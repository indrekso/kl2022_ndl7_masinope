...
# eraldame andmed mudeli õpetamiseks (treenimiseks) ja testimiseks. Juhuslikusatsin juba eelnevalt ridade järjekorra andmestikus, nii et saame siin andmed kaheks eraldada lihtsalt reanumbrite järgi.
...
# otsustamine, milline email on spämm ja milline mitte, käiks sel juhul funktsiooniga `predict`, praegu uusi või muid andmeid meil ei ole, vaatame, kuidas mudel töötab olemasolevate andmetega. NB! Mudeli testimine andmetega, mille põhjal mudel välja töötati, ei ole kindlasti lahendus, mida praktikas kasutada, aga teeme selle näite läbi mõistmaks, miks see pole hea lahendus

knnProgn <- predict(knnModel, newdata = spambase_tst)
knnProgn

# uurime mudeli täpsust treeningandmete põhjal - pole iseenesest masinõppe puhul hea lahendus, aga vaatame selle näitel lihtsalt paari võimalust mudeli täpsuse hindamiseks
